...
interface PhotoDao {
    ...
    // Tổng số ảnh còn chờ resize — ngưỡng foreground cho PhotoResizeWorker
    @Query("SELECT COUNT(*) FROM photos WHERE isResized = 0 AND pathLocal IS NOT NULL")
    suspend fun countPendingResize(): Int
